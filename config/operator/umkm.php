...
if ($aksi === 'tambah') {
  ...
  foreach ($required as $field) {
    if (empty(trim($_POST[$field] ?? ''))) {
      header("Location: ../../operator/pages/data-umkm/tambah_umkm.php?error=field_kosong");
      exit;
    }
  }
  ...
  if (
    ...
    if (!in_array($ext, $allowedExt)) {
      header("Location: ../../operator/pages/data-umkm/tambah_umkm.php?error=format_foto");
      exit;
    }

    if ($_FILES['foto_umkm']['size'] > 2 * 1024 * 1024) {
      header("Location: ../../operator/pages/data-umkm/tambah_umkm.php?error=ukuran_foto");
      exit;
    }
    ...
    if (!move_uploaded_file($_FILES['foto_umkm']['tmp_name'], $path)) {
      header("Location: ../../operator/pages/data-umkm/tambah_umkm.php?error=upload_gagal");
      exit;
    }
  }
  ...
  header("Location: ../../operator/pages/data-umkm/data_umkm.php?msg=added");
  exit;
}
...
if ($aksi == 'edit') {
  ...
  if (!empty($_FILES['foto']['name'])) {
      ...
      move_uploaded_file($tmp, '../../operator/asset/images/umkm/'.$foto);
...
//tanpa foto
  ...
  header("Location: ../../operator/pages/data-umkm/detail_umkm.php?id_umkm=$id_umkm&msg=update");
  exit;
}
...
if (isset($_GET['aksi']) && $_GET['aksi'] === 'hapus') {
  ...
  if (!isset($_GET['id_umkm'])) {
      header("Location: ../../operator/pages/data-umkm/data_umkm.php");
      exit;
  }
  ...
  if ($data) {
      ...
      // Hapus file foto
      if (!empty($data['foto'])) {
          $path = "../../operator/asset/images/umkm/" . $data['foto'];
          if (file_exists($path)) {
              unlink($path);
          }
      }
  ...
  header("Location: ../../operator/pages/data-umkm/data_umkm.php?msg=deleted");
  exit;
}
...
// Jika kosong
if ($search === '') {
  header("Location: ../../operator/pages/data-umkm/data_umkm.php?error=kosong");
  exit;
}
...
if ($jumlah == 0) {
  header("Location: ../../operator/pages/data-umkm/data_umkm.php?search=$search&msg=notfound");
} else {
  header("Location: ../../operator/pages/data-umkm/data_umkm.php?search=$search");
}
exit;
?>